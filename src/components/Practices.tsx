...
type Props = {
  tags: string[]
  data: CollectionEntry<"practices">[]
}

export default function Practices({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [practices, setPractices] = createSignal<CollectionEntry<"practices">[]>([])

  createEffect(() => {
    setPractices(data.filter((entry) =>
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            SHOWING {practices().length} OF {data.length} Practices
          </div>
          <ul class="flex flex-col gap-3">
            {practices().map((practice) => (
              <li>
                <ArrowCard entry={practice} />
              </li>
            ))}
          </ul>
        </div>
